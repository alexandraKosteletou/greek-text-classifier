...
def train_and_eval(df, cfg: TrainConfig, model_path: str | None = None):
    ...
    # Δυναμικό test_size ώστε να «χωράνε» όλες οι κλάσεις στο test όταν γίνεται stratify
    # και μικρό αριθμητικό περιθώριο για float στρογγυλοποιήσεις.
    test_size_float = max(0.2, (k / n) + 1e-9) if n > 0 else 0.2

    # Μπορούμε να κάνουμε stratify μόνο αν κάθε κλάση έχει ≥2 δείγματα
    # και αν το |test| θα έχει τουλάχιστον k δείγματα.
    can_stratify = (
        n >= 2
    ...
    try:
        ...
        # Fallback: χωρίς stratify, με τουλάχιστον 1 δείγμα στο test και 1 στο train
